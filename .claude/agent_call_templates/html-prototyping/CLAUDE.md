...
スキルを内包した専門エージェント。Claude Codeが文脈を判断して自動起動するか、
`@html-prototyper` と明示して呼び出す。

| エージェント名 | 起動条件 | ファイル |
|--------------|---------|---------|
| **html-prototyper** | 「プロトタイプ」「叩き台」「デモ」「お客さんに見せたい」「HTML単体で」「画面イメージを」 | `.claude/agents/html-prototyper.md` |
